...
```{r preNorm, fig.align='center'}
dat.probeMeans <- c(rowMeans(ALL.dat, na.rm = TRUE), 
                    rowMeans(CTRL.dat, na.rm = TRUE))
head(dat.probeMeans)
...
```{r postNorm, fig.align='center', echo=FALSE}
dat.probeMeans <- c(rowMeans(beta.norm[, 1:ncol(ALL.dat)], na.rm = TRUE),
                    rowMeans(beta.norm[, ncol(ALL.dat):ncol(beta.norm)], 
                             na.rm = TRUE))
plotNorm <- rbind(data.frame(plotDat, Norm = "Before"),
                  data.frame(Beta = dat.probeMeans,
                             Dataset = rep(c('ALL', 'CTRL'), 
                                           each = nrow(ALL.dat)), 
                             Norm = "After"))
...
beta.CGI <- aggregate(beta.inCGI, by = list(cginame$cginame), 
                      mean, na.rm = TRUE)
...
```{r limma}
design <- data.frame(Group = relevel(factor(gsub("_[0-9]+", "", 
                                                 colnames(M.CGI))),
                                     ref = "HBC"), 
                     row.names = colnames(M.CGI))
str(design)
...
legend("topright", c("ALL", "HBC"), col = c("darkgoldenrod1", 
                                            "forestgreen"), pch = 15)
par(op)
```
...
```{r stripplot, fig.align='center', fig.height=9, fig.width=9, warning=FALSE, message=FALSE}
...
(beta.DMR5.stripplot <-
   ...
   geom_point(position = position_jitter(width = 0.05), na.rm = TRUE) +
...
```{r coord, fig.align='center', warning=FALSE, message=FALSE}
