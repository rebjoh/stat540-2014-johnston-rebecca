...
if(file.exists("class_LNstatus.Rdata")) { # if previously downloaded
  ...
  save(dat, trainDat, testDat, train.es, test.es, 
       file = "class_LNstatus.Rdata")
}
```
...
ggplot(plotDat, aes(LnStatus, gExp, colour = LnStatus)) + 
  geom_point(alpha = 0.5) +
  facet_wrap(~ gene + Set)
```

```{r include=FALSE}
# # plot 3 randomly picked genes in both training and test sets using lattice
...
pr.err <- matrix(-1, nfold, nmethod, 
                 dimnames = list(paste0("Fold", 1:nfold), 
                                 c("1NN","5NN","10NN", "15NN","LDA","Logit",
                                   "SVM")))
for(i in 1:nfold){
  ...
  pr.log <- predict(m.log, newdata = data.frame(t(testdat.fold)),
                    type = "response")
...
plot(1:nmethod, cv.err, ylim = c(0, 1), xlim = c(1, (nmethod+.5)), type = 'n',
     axes = FALSE, xlab = 'Classifier', ylab = 'Error rate', 
     main = "6-fold CV Error")

for(j in 1:ncol(pr.err))
   points(jitter(rep(j, 6), factor = 2), jitter(pr.err[,j]), cex = 0.8, 
          pch = 'X', col = 'gray')
...
```{r,eval=FALSE}
featureScores <- GeneSelection(X = t(trainDat), y = train.es$LnStatus,
                               learningsets = splits, method = "limma")

# Compare list of selected genes using:
toplist(featureScores)
# We can aggregate the results across the 6 splits.
seliter <- numeric()
for(i in 1:nfold) seliter <- c(seliter, toplist(featureScores, iter = i, 
...
fData(dat)[bestprobes, c("Gene Symbol", "Gene Title", "ENTREZ_GENE_ID",
...
fullFeatureScores <- GeneSelection(X = t(exprs(dat)), 
                                   learningsets = full.learningset, 
                                   y = dat$LnStatus, method = "t.test")

testclassif <- classification(X = t(exprs(dat)), y = dat$LnStatus,
                              learningsets = full.learningset, 
                              genesel = fullFeatureScores, nbgene = 100,
                              classifier = pknnCMA, k = 5)
